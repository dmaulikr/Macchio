...
class AICreatureOld: Creature {
    ...
    var biggerCreaturesNearMe: [Creature] {
        return gameScene.allCreatures.filter { $0 !== self && $0.position.distanceTo(self.position) - $0.radius < AICreatureOld.dangerRange && $0.radius > self.radius * 1.11 }
    }
    
    var smallerCreaturesNearMe: [Creature] {
        return gameScene.allCreatures.filter { $0 !== self && $0.position.distanceTo(self.position) - $0.radius < AICreatureOld.sniffRange && $0.radius * 1.11 < self.radius }
    }
...
//        weak var aicreature: AICreatureOld?
//        var radius: CGFloat {
//            return aicreature?.radius ?? 0
//        }
//        var position: CGPoint {
//            return aicreature?.probeWorldPositionBasedOnAICreatureOld ?? CGPoint(x: 0, y: 0)
//        }
//        func setUpProbe(aiCreature: AICreatureOld) {
//            self.AICreatureOld = AICreatureOld
//        }
//    }
...
//        return false
//    }
//    var probeWorldPositionBasedOnAICreatureOld: CGPoint {
//        let probeX = self.position.x  + cos(targetAngle.degreesToRadians()) * (AICreatureOld.probeDistance + 2 * radius)
//        let probeY = self.position.y + sin(targetAngle.degreesToRadians()) * (AICreatureOld.probeDistance + 2 * radius)
...
//    }
    ...
    func getMinesDetectedByScanner(withAngle scannerAngle: CGFloat, scannerRange: CGFloat = AICreatureOld.scanDistance) -> [GoopMine]{
    ...
    func getWallsDetectedByScanner(withAngle scannerAngle: CGFloat, scannerRange: CGFloat = AICreatureOld.scanDistance) -> [Direction] {
...
//                print("resolving to change angle to nearest orb")
    ...
    func computeNextRunningAwayAction() {
        if minesDetectedByScanner.count > 0 || wallsDetectedByScanner.count > 0 {
            ...
            if position.distanceTo(closestPredator.position) < AICreatureOld.leaveMineRange && canLeaveMine {
                resolveLeaveMine()
            }
        } else {
            resolveChangeStateTo(.EatOrbs)
